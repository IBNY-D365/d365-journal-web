...
# Helper function for dynamic offset account evaluation
def get_offset_account(boa_source_acc):
    ...
    return "B1000002" # Default sidebar value as fallback

# Helper function to generate an empty D365 DataFrame row
def create_base_row():
...
with col1:
    gateway_file = str_lit.file_uploader("1. Upload Zoho / Stripe / Bankcard file (PDF, CSV, XLSX)", type=["csv", "xlsx"])
with col2:
...
# Mock/Load Master Files from local directory or git hierarchy
@str_lit.cache_data
def load_master_files():
    try:
        ...
        str_lit.error(f"Error loading required reference master sheets: {e}")
...
if boa_statement is not None:
    ...
    # Expected BOA columns: 'Date', 'Description', 'Amount', 'Source Account'
    for idx, boa_row in df_boa.iterrows():
        ...
        # ----------------------------------------------------
        # ROUTE 1: ZOHO PAYMENT RULES
        # ----------------------------------------------------
        if "ZOHO" in boa_desc:
            # Placeholder parser for uploaded gateway tracking data
            # Rule: One or more customer credits + 1 aggregated debit fee line
            # Gross - Fees = Net Balance Invariant
            gross_amt = boa_amt * 1.03 # Analytical dummy for demonstration
            ...
            c_row["Account"] = "BC000571" # Derived from Master Lookup
        ...
        # ----------------------------------------------------
        # ROUTE 3: BANKCARD (AUTHORIZE.NET) RULES
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # ROUTE 4: MONTHLY RECURRING EXPENSES TRACK
        # ----------------------------------------------------
        elif "ADOBE INC" in boa_desc or "GENESIS" in boa_desc or "HMF DES" in boa_desc or "MICROSOFT" in boa_desc:
            m_row = create_base_row()
            m_row["Date"] = boa_date
            m_row["Debit"] = abs(boa_amt)
            m_row["Offset account"] = offset_acc
            
            # Rule Exception Handling: Differentiating unique vendor pricing models concurrently
            if "ADOBE INC" in boa_desc and abs(boa_amt) == 826.67:
                ...
                m_row["Description"] = f"Subscription Expense_{boa_row.get('Description', '')}"
                
            output_rows.append(m_row)

        # ----------------------------------------------------
        # ROUTE 5: FALLBACK NON-MONTHLY OPERATIONAL TRACK
        # ----------------------------------------------------
        else:
            f_row = create_base_row()
            f_row["Date"] = boa_date
            # Dynamic Target Fields Left Blank for later manual downstream allocation
            f_row["Account name"] = ""
            f_row["Account type"] = ""
            f_row["Account"] = ""
            f_row["Posting Profile"] = ""
            f_row["Cash code"] = ""
            # Raw description and precise balance preservation invariants
            f_row["Description"] = boa_row.get('Description', '')
            f_row["Debit"] = abs(boa_amt)
            f_row["Offset account"] = offset_acc
            output_rows.append(f_row)

    # Convert mapping schema directly into production DataFrame
    df_result = pd.DataFrame(output_rows, columns=D365_COLUMNS)
    
    # Render Outbound Verification Window
    str_lit.success("Successfully processed automated conversions!")
    str_lit.dataframe(df_result)
    
    # Prepare File Export Byte buffer
    buffer = io.BytesIO()
