...
def parse_zoho_summary_pdf_bulletproof(pdf_file) -> List[ZohoRecord]:
    """Timestamp Chunking Parser - 100% foolproof extraction for all missing rows."""
    records = []
    try:
        reader = PdfReader(pdf_file)
        full_text = ""
        ...
        # Primary Engine: Split precisely by Date/Time stamp
        ...
        if matches:
            for i in range(len(matches)):
                ...
                # CRITICAL FIX: Only requires 1 amount to prevent lines from disappearing!
...
if not (boa_file and zoho_file):
    ...
    if target_form_db:
        ...
        # EXACT column matching to grab correct columns
        biz_col = next((c for c in fdf.columns if 'business name' in c), None)
        if not biz_col:
            biz_col = next((c for c in fdf.columns if 'customer' in c and 'account' not in c), None)
            
        acct_col = next((c for c in fdf.columns if 'customer account' in c or 'account number' in c), None)
    ...
    # CRITICAL FIX: Safe Deduplication that allows multiple identical payments to the same customer!
    ...
    for r in raw_zoho_pool:
        if r.invoice_number:
            ...
            # Allows the two separate $10.00 payments to flow through freely
            zoho_records.append(r)
    ...
    if z_total_gross == 0:
        ...
        # Relaxed BOA mapping: Prevents the system from blocking output
        matched_boa = None
        ...
        if matched_boa:
            ...
            for z_rec in zoho_records:
                ...
                if matched_master_item:
                    ...
                    # CRITICAL FIX: Temporary Receipts MUST default strictly to AR012
                    cash_code = "AR012"
